    ...
    if (!event.isTeam) {
      ...
      try {
        final response = await _repository.getLeaveRequests(
          ...
          search: null,
          status: state.filterCriteria.status ?? state.filterCriteria.statusApprove ?? 'requested',
          statusApprove: state.filterCriteria.statusApprove ?? state.filterCriteria.status ?? 'requested',
